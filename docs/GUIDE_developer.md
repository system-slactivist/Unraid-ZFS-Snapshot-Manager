# GUIDE_developer — Implementation rules for Unraid ZFS Snapshot Manager
...
| ShellCheck Compliance | All scripts must pass shellcheck validation with no errors or warnings |
...
| Duplicating functions between Replication & Restore | Code drift and maintenance overhead | Extract common routines into shared helper files |
...
- **ZFS Dataset Busy / Locked**: If a ZFS dataset replication fails because the target dataset is currently busy or being written to, syncoid will error. Ensure no active writes are performed on the target pool during replication.
